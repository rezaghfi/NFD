                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    NDN_THROW(std::invalid_argument("SMDPStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    NDN_THROW(std::invalid_argument(
      "SMDPStrategy does not support version " + to_string(*parsed.version)));
  }
...
{
  static const auto strategyName = Name("/localhost/nfd/strategy/smdp").appendVersion(4);
  return strategyName;
}

void
SMDPStrategy::afterReceiveInterest(const Interest& interest, const FaceEndpoint& ingress,
...
void
SMDPStrategy::afterNewNextHop(const fib::NextHop& nextHop,
                                   const shared_ptr<pit::Entry>& pitEntry)
{
