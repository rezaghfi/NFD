...
NFD_REGISTER_STRATEGY(SAFStrategy);

NFD_LOG_INIT(SAFStrategy);

const time::milliseconds SAFStrategy::RETX_SUPPRESSION_INITIAL(10);
const time::milliseconds SAFStrategy::RETX_SUPPRESSION_MAX(250);

SAFStrategy::SAFStrategy(Forwarder& forwarder, const Name& name)
  : Strategy(forwarder)
                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    NDN_THROW(std::invalid_argument("SAFStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    NDN_THROW(std::invalid_argument(
      "SAFStrategy does not support version " + to_string(*parsed.version)));
  }
  this->setInstanceName(makeInstanceName(name, getStrategyName()));
}

const Name&
SAFStrategy::getStrategyName()
{
  static const auto strategyName = Name("/localhost/nfd/strategy/saf").appendVersion(4);
  return strategyName;
}

void
SAFStrategy::afterReceiveInterest(const Interest& interest, const FaceEndpoint& ingress,
...
void
SAFStrategy::afterNewNextHop(const fib::NextHop& nextHop,
                                   const shared_ptr<pit::Entry>& pitEntry)
{
