                      ...
                      RETX_SUPPRESSION_MAX)
{
  ...
  if (!parsed.parameters.empty()) {
    NDN_THROW(std::invalid_argument("SoltaniStrategy does not accept parameters"));
  }
  if (parsed.version && *parsed.version != getStrategyName()[-1].toVersion()) {
    NDN_THROW(std::invalid_argument(
      "SoltaniStrategy does not support version " + to_string(*parsed.version)));
  }
...
{
  static const auto strategyName = Name("/localhost/nfd/strategy/soltani").appendVersion(4);
  return strategyName;
}

void
SoltaniStrategy::afterReceiveInterest(const Interest& interest, const FaceEndpoint& ingress,
...
void
SoltaniStrategy::afterNewNextHop(const fib::NextHop& nextHop,
                                   const shared_ptr<pit::Entry>& pitEntry)
{
